...
export async function writeValidatedAST(filePath: string, content: string): Promise<void> {
  // Dynamic import ensures this code path is only exercised in the Tauri runtime,
  // not during Next.js server-side rendering or unit test environments.
  const { invoke } = await import('@tauri-apps/api/core');

  try {
    const newContent = new TextEncoder().encode(content);
    await invoke<void>('apply_manuscript_patch', {
      filePath,
      newContent,
    });
  } catch (err: unknown) {
    const message =
      typeof err === 'string'
        ...
        ? err.message
        : 'Unknown Rust IPC error during atomic write.';

    throw new DiskTransactionError(message, filePath);
  }
}

/**
 * Sends a unified diff patch to the Rust backend to be stored as an immutable audit log.
 * The patch is transmitted natively as a zero-copy Uint8Array.
 *
 * @param filePath - Absolute path to the original `.tex` manuscript.
 * @param patchContent - The unified diff string generated by the LLM.
 * @throws {DiskTransactionError} if the Rust backend reports any I/O failure.
 */
export async function writePatchLog(filePath: string, patchContent: string): Promise<void> {
  ...
  try {
    const patchPayload = new TextEncoder().encode(patchContent);
    await invoke<void>('log_manuscript_patch', {
      filePath,
      patchPayload,
    });
  } catch (err: unknown) {
    const message =
      typeof err === 'string'
        ...
        : err instanceof Error
        ? err.message
        : 'Unknown Rust IPC error during patch logging.';
